...
const AuctionInfo = (props) => {
    ...
    const getMyBidAmount = async() =>{
        await auctionContract.methods.getBidAmount().call({from:account}).then(res=>{
          setMyBidAmount(res)
        })
      }
    useEffect(() => {
  ...
  return (
    <Box mt="10">
        ...
        <Table>
            ...
            <Tr>
                <Th>현재 최고 입찰자</Th>
                <Td>{bidHistory ? `${bidHistory.highestBidder.substr(0,7)} ...... ${bidHistory.highestBidder.substr(38,41)}` : <span> - </span>}</Td>
            </Tr>
            <Tr>
                <Th>현재 입찰가</Th>
                <Td>{bidHistory ? web3.utils.fromWei(bidHistory.highestBindingBid,"ether") : <span> - </span>} ETH</Td>
            </Tr>
            <Tr>
                <Th fontWeight="extrabold">경매에 참여한 내 BNB</Th>
                <Td fontWeight="extrabold">{myBidAmount ? web3.utils.fromWei(myBidAmount,'ether')  : <span> - </span>} ETH</Td>
            </Tr>
        </Table>
    </Box>
  )
