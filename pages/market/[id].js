...
const MarketDetail = () => {
  ...
  const {id, grade, attributes, name, image, description, price } = router.query;
  ...
  const dealNft = async()=>{
    try {
      ...
      await nftDealContract.methods.buyNft(id).send({from:account.toString(), value:web3.utils.toWei(price, "ether")}).then(result=>{
        console.log(result)
        Swal.fire({
          icon: "success",
          title: "Compelete",
