...
const Admin = () => {
  ...
  const isIndeterminate = checkedItems.some(Boolean) && !allChecked
    ...
    const approvedAll = async() =>{
        await gameTokenContract.methods.approveClaim(rankData).send({from:account}).then(result=>{
            console.log(result)
        })
    ...
    const approvedSome = async() =>{
        ...
        //선택한 계정에 한해 클레임 허용
        await gameTokenContract.methods.approveClaim(selectedRankData).send({from:account}).then(result=>{
