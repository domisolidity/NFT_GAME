...
contract Claim_20 is Ownable {
  ...
  */
  ...
  function claim_mission(address _account) external {
    uint rewardAmount = gametoken.allowance(address(this), _account);
    gametoken.transferFrom(address(this), _account, rewardAmount);

    emit ClaimEvent("Mission", _account, rewardAmount, block.timestamp);
  }
  ...
  function approveClaim_rank(WeeklyRank[] memory result) external onlyOwner{
     ...
     for (uint i = 0; i < result.length; i++) {
      ...
      if (result[i].ranking == 1) {
        _reward = top1;
        gametoken.increaseAllowance(result[i].account, _reward);
...
//[["0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db",3,false],["0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db",1,false]]
