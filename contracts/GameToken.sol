...
contract GameToken is BEP20Token {
     using SafeMath for uint256;

    constructor() BEP20Token("DoremiGameToken","DGT",18,1000000000000){} 
    ...
    function victoryClaim(address _account) external payable isReward(_account){
        ...
        // 리워드 받기
        _transfer(owner(), _account, rewardToUser[_account]);   
    }
    ...
    function approveClaim(weeklyRank[] memory result) external{
        ...
        for(uint i = 0; i < result.length; i++){
            if(result[i].ranking == 1){
                increaseAllowance(result[i]._account,top1);
            } else if (result[i].ranking == 2){
                increaseAllowance(result[i]._account,top2);
            } else if (result[i].ranking == 3){
                increaseAllowance(result[i]._account,top3);
            } else {
                increaseAllowance(result[i]._account,top4To10);
            }
        }
    }
    ...
    function rankClaim(address _account) external {
        ...
        uint rewardAmount = allowance(owner(),_account);
        transferFrom(owner(),_account,rewardAmount);
    }
}
