...
contract Auction{
    ...
    event PlaceBidEvent(uint indexed id,address account, uint currentBid,uint highestBindingBid,address highestBidder,uint timestamp);
    event Step(State step);

    constructor(address _eoa, address _nftContractAddres, uint _startBid, uint _remainedBlock,uint _tokenId){
    ...
    function getInitInfo() external view returns(State,uint,uint,uint,uint,uint){
